...
Every cached entry must include metadata alongside the data. `CacheEntry` is a plain in-memory wrapper — not a `HiveObject` — since `CacheService` (Section 5) persists entries as encrypted `Map` values, not via a generated TypeAdapter:
...
class CacheEntry<T> {
  ...
  factory CacheEntry.fromMap(String key, Map map) => CacheEntry(
        key: key,
        data: map['data'] as T,
...
Centralise all Hive cache reads/writes in a single `CacheService`. The box is opened with Hive's AES encryption — the cipher key is generated once and stored in `SecureStorageService`, never hardcoded — because cached entries can contain account/transaction data:
...
final cacheServiceProvider = Provider<CacheService>(
  (ref) => CacheService(ref.watch(secureStorageServiceProvider)),
);
...
class CacheService {
  ...
  Future<Box> get _box async {
    ...
    _cachedBox = await Hive.openBox(_boxName, encryptionCipher: cipher);
    return _cachedBox!;
  }

  Future<List<int>> _encryptionKey() async {
    final stored = await _secureStorage.read(AppConstants.cacheEncryptionKey);
    if (stored != null) return base64Decode(stored);

    final key = Hive.generateSecureKey();
    await _secureStorage.write(
      AppConstants.cacheEncryptionKey,
      base64Encode(key),
    );
    return key;
  }
...
Add `cacheBox` and `cacheEncryptionKey` to `AppConstants`:

```dart
static const String cacheBox = 'cache_box';
static const String cacheEncryptionKey = 'cache_encryption_key';
```
...
- Do not open the cache box without `encryptionCipher` — the cache box is never opened unencrypted
